...
\begin{itemize}[nosep]
    ...
    \item \textbf{Connections}: step where concepts $c_1, c_2$ first share a code element. Note: null model analysis (Section~\ref{sec:cases}) shows that connection counts match random expectation (O/E $= 1.00$); connections should be interpreted as co-occurrence counts rather than discovered relationships.
...
Triadic (3-way) & --- & 74 & --- \\
\midrule
\textbf{Verification} & \textbf{100\% decoder det.} & \textbf{8/16 finite sel.} & \textbf{Crystallization} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Lifecycle.} reptimeline detected 297 bit births and 106 deaths across training, with 45 concept-pair connections (co-occurrence events). 31 of 32 bits were active; 1 bit was permanently dead. A permutation null model (1{,}000 shuffles of bit-concept assignments) shows that connection counts match random expectation (O/E $= 1.00$, $p = 1.0$), indicating that connections are a structural consequence of code density rather than learned relationships.

\textbf{Discovery.} BitDiscovery found 65 anti-correlated dual pairs and 179 dependency edges. Critically, dependency edges are $4.3\times$ above random expectation (179 real vs.\ 41.3 expected from null model), confirming that the autoencoder learns genuine directed bit-prediction structure beyond what code density alone explains.

\textbf{Decoder determinism verification.} A \emph{code swap} experiment tested whether the 32-bit code determines the decoder output: for each pair of digits $(d_i, d_j)$, we encoded $d_i$'s image but decoded using $d_j$'s binary code. The decoder produced $d_j$'s reconstruction in 100\% of cases ($n = 100$ swaps), confirming that the decoder is a pure function of the discrete code. Note that this verifies decoder determinism---that $f(\text{code}) \to \text{output}$ is consistent---not causal influence of inputs on codes (Figure~\ref{fig:code_swap}).

\begin{figure}[h]
\centering
\includegraphics[width=0.75\columnwidth]{figures/code_swap.png}
\caption{MNIST code swap decoder determinism test. Decoding with digit $X$'s 32-bit code always produces digit $X$'s reconstruction, regardless of the input image. This confirms the decoder is a pure function of the code.}
\label{fig:code_swap}
\end{figure}
...
\textbf{Setup.} We used EleutherAI's pre-trained sparse autoencoder (\texttt{sae-pythia-70m-32k}) on layer~3 MLP outputs of Pythia-70M~\citep{biderman2023pythia}. We defined 60 probe concepts across 6 semantic domains (physical, animals, emotions, abstract, social, properties) and tracked 256 SAE features (selected by activation frequency at the final checkpoint) across 12 training checkpoints from step~0 to step~143{,}000.

\textbf{Lifecycle.} reptimeline detected 1{,}835 births, 875 deaths, and 1{,}770 connections (co-occurrence events; O/E $= 1.00$ vs.\ null model), with 2 phase transitions detected automatically. 64 of 256 tracked features were active (the remaining 192 were effectively dead---expected given SAE sparsity).

\textbf{Discovery.} BitDiscovery found 34 dual pairs, 227 dependency edges, and 74 three-way AND-gate interactions before correction. After Bonferroni adjustment for multiple comparisons, 66 dependencies survived---infinitely above random expectation (0 expected from null model with 1{,}000 permutations), providing the strongest evidence that real data contains genuine directed structure. Auto-labeling via embedding centroids produced interpretable names: ``teacher,'' ``belief,'' ``dog,'' ``wisdom,'' ``ocean.'' Figure~\ref{fig:swimlane} shows the swimlane heatmap for the first 20 features across 12 checkpoints.
...
Connection tracking$^\dagger$ & \checkmark & --- & --- & --- & --- \\
...
\noindent$^\dagger$Connection counts match random expectation (O/E $= 1.00$) on all three backends (MNIST, Pythia, and TriadicGPT with 72 primitives over 20 checkpoints, 1000 permutations); they function as co-occurrence counts rather than discovered relationships. Dependencies (directed bit predictions) are the validated discovery metric ($4.3\times$ and $\infty$ above random, respectively).
...
reptimeline fills a gap in the discrete representation toolbox. It reveals training dynamics that scalar logging misses (crystallization events), discovers interpretable structure without supervision (duals, dependencies, three-way interactions), and verifies that discovered features are causally meaningful---with rigorous statistical methodology including bootstrap confidence intervals, permutation tests, Benjamini-Hochberg FDR correction, and Cohen's $d$ effect sizes (8/16 with finite KL selectivity at 26.8$\times$ mean on Pythia-70M, 100\% decoder determinism on MNIST). Null model validation reveals that not all discovered metrics are equally informative: connection counts match random expectation (O/E $= 1.00$ on all three backends), while dependencies are strongly above chance ($4.3\times$ on MNIST, infinitely above on Pythia). We report this honestly as evidence that dependency structure---not co-occurrence---is the primary validated discovery of the BitDiscovery module.

\textbf{Availability.} \texttt{pip install reptimeline}. Source code, examples, and pre-computed results are available at \url{https://github.com/arturoornelasb/reptimeline} (software DOI: \href{https://doi.org/10.5281/zenodo.19208628}{10.5281/zenodo.19208628}). Licensed under BUSL-1.1 (free for research; converts to AGPL-3.0 in 2030). The package includes 224~unit tests at 91\% coverage, 11~example pipelines, 6~visualization modules (5~static, 4~interactive), 3~built-in extractors (VQ-VAE, SAE, FSQ), JSON/CSV export, a command-line interface, and a structured exception hierarchy. All statistical methods are implemented in pure NumPy with no SciPy dependency.

\textbf{Companion work.} This work is part of a four-paper program on triadic neurosymbolic representations. The present paper's claims are evaluated independently; companion results provide additional context but are not required for the conclusions drawn here. The companion papers address the algebraic encoding formalism (\href{https://doi.org/10.5281/zenodo.19205805}{doi:10.5281/zenodo.19205805}), a learnable projection head for language models (\href{https://doi.org/10.5281/zenodo.19206545}{doi:10.5281/zenodo.19206545}), and an ontological framework of semantic dualities (\href{https://doi.org/10.5281/zenodo.19375167}{doi:10.5281/zenodo.19375167}).
...
J.~A. Ornelas~Brand.
\newblock Triadic Neurosymbolic Engine: Prime factorization as a neurosymbolic bridge: Projecting continuous embeddings into discrete algebraic space for deterministic verification.
\newblock 2026.
...
J.~A. Ornelas~Brand.
\newblock End-to-end prime factorization in a generative language model: Learned algebraic encoding from joint training.
\newblock 2026.
